# ADHD: Kompleksowy Przewodnik 360° - Instrukcje dla Claude

> **WAŻNE - Edycja tego pliku:**
> Stosuj zasadę **KISS (Keep It Simple, Stupid)** przy aktualizacji tego dokumentu.
> - Nie over-engineeruj instrukcji
> - Nie dodawaj zbędnej complexity
> - Nie marnuj tokenów na nadmiarowe wytyczne
> - Prostota i użyteczność > perfekcja i sztywność

## Kontekst Projektu

Tworzę polską książkę o ADHD z interdyscyplinarnym, evidence-based podejściem. Książka ma być kompleksowym przewodnikiem łączącym:
- Naukę (neurobiologia, psychiatria, psychologia)
- Praktykę (techniki, strategie, narzędzia)
- Nowoczesność (biohacking, personalizacja, nootropiki)

**Lokalizacja:** `/home/user/adhd-complete-guide/`
**Język:** Polski
**Ton:** Profesjonalny ale przystępny, oparty na dowodach, bez sensacji

## Struktura Książki (20 Rozdziałów)

### CZĘŚĆ I: Zrozumieć ADHD (Rozdziały 1-5)

**Rozdział 1: Co to właściwie jest ADHD?**
- Definicja, kryteria diagnostyczne DSM-5 i ICD-11
- Mity vs fakty
- ADHD u dzieci vs dorosłych
- Spektrum objawów: nieuwaga, nadpobudliwość, impulsywność
- Podtypy: predominant inattentive, hyperactive-impulsive, combined

**Rozdział 2: Neurobiologia ADHD - Jak Działa Mózg**
- Struktury mózgu: prefrontal cortex, basal ganglia, cerebellum
- Neurotransmitery: dopamina, norepinefryna, serotonina
- Sieci neuronalne: default mode network, executive function network
- Neuroplastyczność i ADHD
- Różnice strukturalne i funkcjonalne (fMRI, PET)

**Rozdział 3: Genetyka i Czynniki Środowiskowe**
- Dziedziczność ADHD (70-80%)
- Geny kandydujące (DRD4, DRD5, DAT1)
- Czynniki prenatalne i perinatalne
- Wpływ toksyn środowiskowych
- Epigenetyka

**Rozdział 4: Diagnoza ADHD**
- Proces diagnostyczny krok po kroku
- Narzędzia diagnostyczne (ASRS, DIVA, Conners)
- Wywiad kliniczny
- Testy neuropsychologiczne
- Diagnoza różnicowa (depresja, lęk, autyzm, bipolarne)
- ADHD u kobiet - specyfika diagnostyczna

**Rozdział 5: Współwystępujące Zaburzenia**
- Depresja i lęk
- Zaburzenia snu
- Uzależnienia
- Zaburzenia osobowości
- Spektrum autyzmu
- Zaburzenia nastroju

### CZĘŚĆ II: Leczenie Farmakologiczne (Rozdziały 6-8)

**Rozdział 6: Stymulujące Leki (Methylphenidate, Amphetamines)**
- Mechanizmy działania
- Rodzaje: krótko- i długodziałające
- Dawkowanie i titracja
- Skuteczność (effect sizes)
- Działania niepożądane
- Mity o uzależnieniu

**Rozdział 7: Leki Niestymulujące (Atomoxetine, Guanfacine, Bupropion)**
- Kiedy rozważać niestymulujące?
- Mechanizmy działania
- Skuteczność vs stymulujące
- Profil działań niepożądanych
- Kombinacje leków

**Rozdział 8: Personalizacja Farmakoterapii**
- Farmakogenetyka (CYP2D6, inne)
- Dobór leku do profilu objawów
- Zarządzanie działaniami niepożądanymi
- Okresy bez leku (drug holidays)
- Monitoring i dostosowywanie
- Co robić gdy leki nie działają?

### CZĘŚĆ III: Psychologia i Terapia (Rozdziały 9-11)

**Rozdział 9: Funkcje Wykonawcze - Rdzeń Problemu**
- Co to są funkcje wykonawcze?
- Working memory (pamięć robocza)
- Hamowanie impulsów (inhibition)
- Elastyczność poznawcza (cognitive flexibility)
- Planowanie i organizacja
- Regulacja emocji
- Jak ADHD wpływa na każdą z nich

**Rozdział 10: Terapia Poznawczo-Behawioralna (CBT)**
- CBT specyficzne dla ADHD
- Restrukturyzacja poznawcza
- Techniki behawioralne
- Zarządzanie prokrastynacją
- Praca z perfekjonizmem
- Evidence base i skuteczność

**Rozdział 11: Inne Podejścia Terapeutyczne**
- DBT (Dialectical Behavior Therapy) dla regulacji emocji
- Mindfulness i ADHD
- Coaching ADHD
- Terapia rodzinna i par
- Grupy wsparcia
- Neurofeedback - co mówią badania?

### CZĘŚĆ IV: Praktyczne Strategie (Rozdziały 12-14)

**Rozdział 12: Systemy i Organizacja**
- External brain: narzędzia i systemy
- Time management: techniki (Pomodoro, time boxing)
- Task management: GTD, Bullet Journal, cyfrowe narzędzia
- Środowisko pracy: minimalizacja rozpraszaczy
- Rutyny i nawyki
- Body doubling i accountability

**Rozdział 13: Zarządzanie Energią i Uwagą**
- Hyperfocus vs distractibility
- Praca z naturalnym rytmem energii
- Breaks i recovery
- Context switching
- Zarządzanie przeciążeniem sensorycznym
- Znajdowanie "flow state"

**Rozdział 14: Emocje, Relacje, Komunikacja**
- Dysregulacja emocjonalna w ADHD
- Rejection sensitivity dysphoria (RSD)
- Komunikacja w związkach
- ADHD w pracy - disclosure i accommodations
- Parenting z ADHD
- Przyjaźnie i życie społeczne

### CZĘŚĆ V: Biohacking i Optymalizacja (Rozdziały 15-18)

**Rozdział 15: Dieta i Odżywianie**
- Dieta a objawy ADHD - co mówią badania?
- Proteiny i stabilizacja glukozy
- Omega-3 fatty acids
- Mikrobiom jelitowy
- Eliminacja: gluten, cukier, additives
- Suplementacja podstawowa

**Rozdział 16: Aktywność Fizyczna**
- Exercise as medicine dla ADHD
- Typy aktywności: aerobic, strength, coordination
- Dawkowanie i frequency
- Exercise timing (rano vs wieczór)
- Sport jako terapia
- Ruch w ciągu dnia

**Rozdział 17: Sen i Rytmy Dobowe**
- ADHD i problemy ze snem (delayed sleep phase)
- Sleep hygiene 2.0
- Chronotypy
- Melatonina
- Light exposure i circadian rhythm
- Naps i recovery sleep

**Rozdział 18: Nootropiki i Suplementy**
- Evidence-based suplementacja:
  - L-tyrozine, L-theanine
  - Caffeine - pros and cons
  - Rhodiola, Ashwagandha
  - Magnesium, Zinc, Iron
  - Phosphatidylserine
  - Alpha-GPC, CDP-choline
- Kombinacje i synergies
- Timing i dawkowanie
- Bezpieczeństwo i interakcje z lekami
- Co NIE działa (snake oil alert)

### CZĘŚĆ VI: Całościowe Podejście (Rozdziały 19-20)

**Rozdział 19: Integracja - Twój Personalny Protokół**
- Assessment: gdzie jesteś teraz?
- Priorytetyzacja interwencji
- Building your stack: leki + terapia + lifestyle
- Eksperymenty n=1
- Tracking i monitoring
- Iteracja i dostosowanie
- Long-term sustainability

**Rozdział 20: ADHD jako SuperPower?**
- Strengths-based perspective
- Kreatywność i out-of-box thinking
- Hyperfocus jako asset
- Entrepreneurship i ADHD
- Flourishing, not just functioning
- Community i resources
- Przyszłość badań nad ADHD

## Zasady Pisania

### Ton i Styl
- **Profesjonalny ale przystępny**: wyjaśniaj terminy naukowe, używaj analogii
- **Evidence-based**: zawsze wskazuj źródła badań
- **Zbalansowany**: pokazuj co działa, ale też czego unikać
- **Bez hype'u**: ani demonizowanie ADHD, ani gloryfikacja
- **Praktyczny**: każdy rozdział kończy się "Key Takeaways" i "Action Steps"

### Struktura Rozdziału (OBOWIĄZKOWA)
1. **Hook/Opening** - BEZPOŚREDNI zwrot do czytelnika w drugiej osobie
   - ❌ **NIE**: "Anna ma 32 lata i zmaga się z ADHD..." (wymyślone postacie)
   - ✅ **TAK**: "Czy też miałeś tak, że zanim zorientowałeś się że masz ADHD, szukałeś sam środków które okiełznają Twój mózg? Może próbowałeś nootropików, może czytałeś o biohackingu..."
   - Cel: czytelnik utożsamia się z doświadczeniem, czuje że jest rozumiany

2. **Nauka** - wyjaśnienie mechanizmów, badania
   - Używaj analogii i prostego języka
   - Cytuj konkretne badania z nazwiskami, rokiem, effect sizes

3. **Praktyka** - konkretne techniki, protokoły
   - Actionable, step-by-step
   - Dawkowanie, timing, konkretne liczby

4. **Przykłady z Życia** - 2-3 realistyczne scenariusze
   - Real-world applications
   - Różne profile (student, pracownik, rodzic)

5. **Kluczowe Wnioski** - 5-7 bulletów podsumowujących

6. **Kroki do Działania** - konkretna lista kroków
   - Podziel na timeframes (24h, tydzień, miesiąc)

7. **Zasoby** - polecane narzędzia, aplikacje, książki

8. **Bibliografia** - OBOWIĄZKOWA sekcja
   - Format: APA style z pełnymi cytowaniami
   - Min. 8-12 źródeł naukowych
   - DOI links gdzie możliwe
   - Przykład:
     ```
     1. **Cortese, S., Kelly, C., Chabernaud, C., Proal, E., Di Martino, A., Milham, M. P., & Castellanos, F. X.** (2012). Toward systems neuroscience of ADHD: A meta-analysis of 55 fMRI studies. *American Journal of Psychiatry*, *169*(10), 1038-1055. https://doi.org/10.1176/appi.ajp.2012.11101521
     ```

### Długość
- **Optymalna**: 3,500-4,500 słów na rozdział
- **Minimum**: 2,500 słów
- **Akceptowalne**: do 6,000 słów dla kompleksowych tematów (np. nootropiki, farmakoterapia)
- **Priorytet**: głębia merytoryczna > długość, ale nie za krótko

### Cytowania i Źródła
- **W tekście**: Wskazuj badania z nazwiskami, rokiem, kluczowymi wynikami
  - Przykład: "Cortese et al. (2018) w meta-analizie obejmującej 133 badania wykazali..."
- **Nazwiska ekspertów**: Dr. Russell Barkley, Dr. Hallowell, etc.
- **Effect sizes**: Zawsze podawaj gdy dostępne (d = 0.45, OR = 1.8, etc.)
- **Bibliografia na końcu**: Pełne cytowania w formacie APA z DOI

### Do Unikania
- Oversimplification złożonych zagadnień
- Obiecywanie "cure" lub "quick fixes"
- Ignorowanie individual variability
- Promowanie nielegalnych substancji
- Medical advice (zawsze: "skonsultuj z lekarzem")

## Task List - Postęp

### ✅ Infrastruktura
- [x] Utworzenie repo
- [x] Struktura katalogów
- [x] README.md
- [x] claude.md

### 📝 Rozdziały - Status

**CZĘŚĆ I: Zrozumieć ADHD**
- [ ] Rozdział 1: Co to właściwie jest ADHD? (0/4000 słów)
- [ ] Rozdział 2: Neurobiologia ADHD (0/4000 słów)
- [ ] Rozdział 3: Genetyka i Czynniki Środowiskowe (0/3500 słów)
- [ ] Rozdział 4: Diagnoza ADHD (0/4000 słów)
- [ ] Rozdział 5: Współwystępujące Zaburzenia (0/3500 słów)

**CZĘŚĆ II: Leczenie Farmakologiczne**
- [ ] Rozdział 6: Stymulujące Leki (0/4000 słów)
- [ ] Rozdział 7: Leki Niestymulujące (0/3500 słów)
- [ ] Rozdział 8: Personalizacja Farmakoterapii (0/4000 słów)

**CZĘŚĆ III: Psychologia i Terapia**
- [ ] Rozdział 9: Funkcje Wykonawcze (0/4000 słów)
- [ ] Rozdział 10: CBT dla ADHD (0/4000 słów)
- [ ] Rozdział 11: Inne Podejścia Terapeutyczne (0/3500 słów)

**CZĘŚĆ IV: Praktyczne Strategie**
- [ ] Rozdział 12: Systemy i Organizacja (0/4000 słów)
- [ ] Rozdział 13: Zarządzanie Energią i Uwagą (0/3500 słów)
- [ ] Rozdział 14: Emocje, Relacje, Komunikacja (0/4000 słów)

**CZĘŚĆ V: Biohacking i Optymalizacja**
- [ ] Rozdział 15: Dieta i Odżywianie (0/4000 słów)
- [ ] Rozdział 16: Aktywność Fizyczna (0/3500 słów)
- [ ] Rozdział 17: Sen i Rytmy Dobowe (0/4000 słów)
- [ ] Rozdział 18: Nootropiki i Suplementy (0/4500 słów)

**CZĘŚĆ VI: Całościowe Podejście**
- [ ] Rozdział 19: Integracja - Twój Personalny Protokół (0/4000 słów)
- [ ] Rozdział 20: ADHD jako SuperPower? (0/3500 słów)

### 📚 Materiały Dodatkowe
- [ ] Słowniczek terminów medycznych i naukowych
- [ ] Bibliografia z kategoryzacją (badania, książki, blogi)
- [ ] Appendix: Narzędzia diagnostyczne
- [ ] Appendix: Resources (aplikacje, strony, communities)
- [ ] Index

## Kluczowe Źródła

### Badacze i Klinicyści
- **Dr. Russell Barkley** - światowy autorytet ADHD, teoria executive dysfunction
- **Dr. Edward Hallowell & Dr. John Ratey** - "Driven to Distraction"
- **Dr. Thomas Brown** - Brown ADD Scales, model funkcji wykonawczych
- **Dr. Ari Tuckman** - praktyczne podejście, książki self-help
- **Dr. William Dodson** - RSD (Rejection Sensitivity Dysphoria)

### Kluczowe Badania
- ADHD heritability studies (Faraone et al.)
- Neuroimaging studies (Cortese et al., Shaw et al.)
- Medication meta-analyses (Cortese et al., 2018)
- Non-pharmacological interventions (Sonuga-Barke et al.)

### Blogi i Resources
- ADDitude Magazine
- CHADD (Children and Adults with ADHD)
- How to ADHD (Jessica McCabe)
- Dr. Russell Barkley's lectures (YouTube)

## Instrukcje dla Claude

1. **Przed edycją**: Zawsze czytaj odpowiedni plik rozdziału
2. **Cytowania**: Włączaj references do badań i ekspertów
3. **Praktyczność**: Każdy rozdział musi mieć actionable insights
4. **Balans**: Nauka + praktyka w każdym rozdziale
5. **Język**: Polski, ale terminy ang. w nawiasach przy pierwszym użyciu
6. **Commit messages**: Opisowe, wskazujące co zostało dodane/zmienione
7. **Nie wymyślaj badań**: Jeśli nie masz pewności o źródle, zaznacz jako "research needed"

## Git Workflow

- **Main branch**: production-ready chapters
- **Feature branches**: `claude/chapter-X-topic`
- **Commit style**: "Dodaj sekcję o [temat] (Rozdział X)" lub "Rozszerz [temat] o [szczegóły]"

## Rozpocznij Od

Gdy dostaniesz zadanie, zacznij od najważniejszych rozdziałów:
1. Rozdział 2 (Neurobiologia) - fundament zrozumienia
2. Rozdział 1 (Co to jest ADHD) - overview
3. Rozdział 9 (Funkcje wykonawcze) - praktyczne implikacje
4. Rozdział 12 (Systemy) - immediate value

---

## Lessons Learned - Best Practices

### Na podstawie ukończonej pracy (20/20 rozdziałów):

#### 1. Bibliografia
- **Dodawaj od razu**: Najlepiej pisać bibliografię razem z rozdziałem, nie później
- **Format spójny**: Używaj zawsze pełnego APA (nazwiska wszystkich autorów, DOI, pełne nazwy czasopism)
- **Liczba źródeł**: 10 źródeł to sweet spot (nie za mało, nie przytłaczające)
- **Jakość > ilość**: Lepiej 8 wysokiej jakości źródeł niż 15 średnich

#### 2. Hooki i Opening
- **Bezpośredni adres**: "Czy też miałeś tak że..." działa lepiej niż wymyślone postacie
- **Relatable experiences**: Opisuj sytuacje które czytelnik prawdopodobnie przeżył
- **Unikaj cliché**: Nie "Anna ma 32 lata", nie "Jan zawsze zapominał kluczy"
- **Autentyczność**: Czytelnik z ADHD rozpozna fałszywe scenariusze

#### 3. Długość rozdziałów
- **Rozdziały 1-13 były systematycznie krótsze** (2000-2800 słów) - to za mało
- **Optymalna długość**: 3,500-4,500 słów daje wystarczająco dużo miejsca na:
  - Wyjaśnienie nauki
  - Praktyczne protokoły
  - Przykłady
  - Bibliografię
- **Rozdziały kompleksowe** (farmakologia, nootropiki) mogą mieć 5,000-6,000 słów

#### 4. Struktura
- **Przykłady z życia są kluczowe**: 2-3 szczegółowe case studies robią różnicę
- **Konkretne liczby**: Dawkowanie, timing, frequencies - nie "trochę", ale "200mg", "30 minut przed"
- **Tabele są helpful**: Porównania leków, stacki nootropików, schedule
- **Kroki do działania**: Podzielone na timeframes (24h/tydzień/miesiąc) są bardziej actionable

#### 5. Tematy niszowe/zaawansowane
- **Research chemicals sekcja** w rozdziale nootropików była dobrym pomysłem
- **Ostrzeżenia są ważne**: Jasno zaznaczaj ryzyka, nie tylko korzyści
- **Balanced perspective**: Nie gloryfikuj experimental compounds

#### 6. Commit Strategy
- **Częste commity**: Lepiej commit po każdym rozdziale niż batch
- **Opisowe messages**: "Rozszerzono Rozdział 18: Tier 4 nootropiki" > "Update chapter 18"
- **Bibliografia osobno**: Warto commitować bibliografie jako osobny task gdy się je dodaje później

#### 7. Git Hooks i Workflow
- **Stop-hook sprawdza uncommitted changes**: Pamiętaj commit + push przed końcem sesji
- **Branch naming**: `claude/adhd-guide-chapter-14-WmIL4` - opisowy + session ID
- **Push failures**: Retry z exponential backoff (2s, 4s, 8s, 16s)

#### 8. Co UNIKAĆ
- ❌ Wymyślonych postaci w hookach (Anna, Marek, Karolina)
- ❌ Za krótkich rozdziałów (<2,500 słów)
- ❌ Braków w bibliografii
- ❌ Ogólników bez konkretów ("niektóre badania pokazują...")
- ❌ Promises bez evidenece ("to na pewno pomoże każdemu")

#### 9. Co ZAWSZE robić
- ✅ Bezpośredni zwrot do czytelnika (ty, twoje, twój mózg)
- ✅ Effect sizes w wynikach badań
- ✅ Pełne nazwiska autorów w cytowaniach
- ✅ DOI links w bibliografii
- ✅ Konkretne protokoły z liczbami
- ✅ Ostrzeżenia o ryzykach i interakcjach
- ✅ Sekcja "Co NIE działa" (snake oil alert)

---

**Ostatnia aktualizacja:** 2025-12-29
**Status:** ✅ Projekt ukończony - 20/20 rozdziałów + pełne bibliografie
**Commits:** 76e188d (bibliografie 1-6, 19), d7701d8 (Rozdział 18 Tier 4), 20eb7ed (README links)
